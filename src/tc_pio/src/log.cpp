...
void PIO::loggingCallback()
{
}

void PIO::coordLogging(char* strname)
{
}

void PIO::tacktimeLogging(char* strname, char* strdata)
{
...
void PIO::logErr(int nErr)
{
	...
	logSys(TrShareEnum::log_err,"Err = %d, Description = %s, Div(Warning_0, Heav_1, ByOp_2) = %d ", 
												nErr, err_info_.klist[nErr].strName, err_info_.klist[nErr].nDiv );
	...
	string strErrName(err_info_.klist[nErr].strName);
	int nDiv = err_info_.klist[nErr].nDiv;
...
//받은 task_logging 데이터를 기록한다.
void PIO::taskLogging(Tacklist& _tacklist)
{
...
void PIO::logSys(int _Type, const char *frm,...)
{
	string strName[] = {"PIO_SYSTEM","PIO_COORD","ERR_PIO","PIO_TACKTIME","LOG_PIO","PIO_GUI", "PIO_MOTOR","PIO_HOST","PIO_BMS","PIO_HYBRID","PIO_IO","PIO_DATA"};
...
std::string PIO::logCreateDirectory(string foldername)
{
...
int PIO::rmdirs(const char *path, int force)
{
    DIR *  dir_ptr      = NULL;
...
// }
void PIO::logRemoveFolder(const char *path)
{
    ...
    while((file = readdir(dir_ptr)) != NULL) {
		...
		if(tm_day > 30)
		{
			...
			{
				RCLCPP_ERROR(get_logger(),"call log_remove_folder = Error occurred while deleting file");
				continue;
			}
		}
	}
...
unsigned long PIO::elaspseTime(unsigned int dw_start_tick_count)
{
    unsigned int dwTickCount = getTickCount();
             
    if( dw_start_tick_count > dwTickCount)        
        return ((dwTickCount - dw_start_tick_count) + (unsigned int) 0xffffffff);
           
    return (dwTickCount - dw_start_tick_count);    
}
//시간을 셋팅하고, 원하는 시간을 넣어 그 시간을 확인 한다.
bool PIO::getTimeOut(unsigned int  dw_start_tick_count, int umillisecound)
{
    if(elaspseTime(dw_start_tick_count) >= umillisecound ) return true;
    else return false;
}
