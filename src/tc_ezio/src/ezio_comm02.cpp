...
Ezio2::~Ezio2()
{
        ...
        RCLCPP_INFO(get_logger(), "~Ezio2");
}
//변수 생성 하기.
void Ezio2::initialCreate()
{
...
//파라미터 리셋
void Ezio2::initialPara()
{
        send_counter_ = 0;
}

//소켓 통신 설정.
bool Ezio2::initialComm()
{
        ...
        if (inet_pton(AF_INET, "192.168.192.11", &st_srv_addr_.sin_addr) <= 0) {
...
//서버에 메세지 보내기.
void Ezio2::srvCommandSend(int task)
{
...
bool Ezio2::srvDataReceiv()
{
        ...
        //                                                2, chbyte1, chbyte2, chbyte3,chbyte4, chbyte5, 
...
void Ezio2::returnDataIoStatus(int &data_01, int &data_02, int &data_03, int &data_04)
{
        data_01 = wd_data_[0];
        data_02 = wd_data_[1];
        data_03 = wd_data_[2];
...
//서버에 메세지 보내기.
bool Ezio2::srvCommandSend(int set_data_01, int set_data_02, int set_data_03, int set_data_04,
                                int reset_01, int reset_02 ,int reset_03 ,int reset_04 )
{
	...
	sBuffer[4] = 0xC6; // OUTPUT WRITE 명령어.
        ...
        if (return_set_out_())
        {
                ...
                if (data_01 == set_data_01 && data_02 == set_data_02 && data_03 == set_data_03 && data_04 == set_data_04) 
                {
...
//io output 출력 변경을 한다.
void Ezio2::bitStateToBlockState(int nModID, int nBlockID, int nbitNo, int state)
{ 
...
bool Ezio2::outEzioSetting()
{
        srvCommandSend(stbit_info_.nInfo[0].nSetData,
                        stbit_info_.nInfo[1].nSetData,
                        stbit_info_.nInfo[2].nSetData,
                        stbit_info_.nInfo[3].nSetData,
