...
Ezio1::~Ezio1()
{
        ...
        RCLCPP_INFO(get_logger(), "~Ezio1");
}
//변수 생성 하기.
void Ezio1::initialCreate()
{
...
//파라미터 리셋
void Ezio1::initialPara()
{
        send_counter_ = 0;
}

//소켓 통신 설정.
bool Ezio1::initialComm()
{
        ...
        if (inet_pton(AF_INET, "192.168.192.10", &st_srv_addr_.sin_addr) <= 0) {
...
//서버에 메세지 보내기.
void Ezio1::srvCommandSend(int task)
{
...
bool Ezio1::srvDataReceiv()
{
        ...
        //                                                1, chbyte1, chbyte2, chbyte3,chbyte4, chbyte5, 
...
void Ezio1::returnDataIoStatus(int &data_01, int &data_02, int &data_03, int &data_04)
{
        data_01 = wd_data_[0];
        data_02 = wd_data_[1];
        data_03 = wd_data_[2];
...
//서버에 메세지 보내기.
bool Ezio1::srvCommandSend(int set_data_01, int set_data_02, int set_data_03, int set_data_04,
...
//io output 출력 변경을 한다.
void Ezio1::bitStateToBlockState(int nModID, int nBlockID, int nbitNo, int state)
{ 
