...
Ezio4::~Ezio4()
{
        ...
        RCLCPP_INFO(get_logger(), "~Ezio4");
}
//변수 생성 하기.
void Ezio4::initialCreate()
{
...
//파라미터 리셋
void Ezio4::initialPara()
{
        send_counter_ = 0;
}

//소켓 통신 설정.
bool Ezio4::initialComm()
{
        ...
        if (inet_pton(AF_INET, "192.168.192.14", &st_srv_addr_.sin_addr) <= 0) {
...
//서버에 메세지 보내기.
void Ezio4::srvCommandSend(int task)
{
	...
	sBuffer[4] = task;
        ...
        // RCLCPP_INFO(get_logger(), "CommandSendId=%d, %d, %d, %d, %d", 
        //                                         sBuffer[0], sBuffer[1], sBuffer[2],sBuffer[3], sBuffer[4]);
}
//서버에서 데이터 리턴 받기.
bool Ezio4::srvDataReceiv()
{
        ...
        //                                                4, chbyte1, chbyte2, chbyte3,chbyte4, chbyte5, 
...
void Ezio4::returnDataIoStatus(int &data_01, int &data_02, int &data_03, int &data_04)
{
        data_01 = wd_data_[0];
        data_02 = wd_data_[1];
        data_03 = wd_data_[2];
...
//서버에 메세지 보내기.
bool Ezio4::srvCommandSend(int set_data_01, int set_data_02, int set_data_03, int set_data_04,
                                int reset_01, int reset_02 ,int reset_03 ,int reset_04 )
{
	...
	sBuffer[4] = 0xC6; // OUTPUT WRITE 명령어.
        ...
        if (return_set_out_())
        {
                ...
                // RCLCPP_INFO(get_logger(), "Out Data Id=%d, %d, %d, %d, %d, %d, %d, %d", 
                //                                        data_01, data_02, data_03, data_04,set_data_01, set_data_02, 
                //                                        set_data_03, set_data_04);
        }
        else
        {
                return false;
...
bool Ezio4::return_set_out_()
{
        ...
        if( recv(socket_client_fd_,rev_data_, FD_MAX_SIZE, 0 ) <= 0 )
        {  
                RCLCPP_INFO(get_logger(), "Return Receive Failed ... ");	              
                return false;
        }
...
//io output 출력 변경을 한다.
void Ezio4::bitStateToBlockState(int nModID, int nBlockID, int nbitNo, int state)
{ 
